...
def load_tokenized_dataset(prefix):
    """Load dataset đã tokenize từ ổ cứng tương ứng với prefix"""
...
def load_model_and_tokenizer(prefix):
    """Load model final tương ứng với từng prefix"""
...
def save_detailed_evaluation(predictions, labels, prefix):
    """Hàm bổ sung để vẽ Confusion Matrix và in Classification Report chi tiết (Table 3 & 4)"""
    roberta_eval_dir = os.path.join(EVAL_DIR, "roberta")
    os.makedirs(roberta_eval_dir, exist_ok=True)
    
    # Load class mapping từ file CSV test để gán nhãn cho biểu đồ
...
def main():
    ...
    for prefix, batch_size in configs:
        ...
        # 1. Chạy evaluate cơ bản cho cả 3 tập (giữ nguyên logic gốc của bạn)
        for split in ["train", "validation", "test"]:
            print(f"Evaluating on {split} set...")
            metrics = trainer.evaluate(eval_dataset=dataset[split])
            save_metrics(metrics, prefix, split)

        # 2. Chạy predict riêng cho tập test để lấy biểu đồ và báo cáo chi tiết
