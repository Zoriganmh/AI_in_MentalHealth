...
from config import (
    DATA_DIR,      # Dùng DATA_DIR để tự động nối chuỗi thay vì fix cứng TRAIN_PATH
    LABEL_COL,
    FEATURES_DIR,
)
...
def tokenize_dataset(dataset, tokenizer, text_col, label_col, max_length):
    ...
    # Chỉ rename nếu cột chưa được rename (tránh lỗi nếu chạy lại)
...
def main():
    ...
    for prefix, text_col, max_len in configs:
        print(f"\n{'='*50}")
        print(f"PROCESSING DATASET: {prefix.upper()}")
        print(f"{'='*50}")
    ...
    print("\nDone. All tokenized datasets saved.")
