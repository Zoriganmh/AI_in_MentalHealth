...
def load_model_and_tokenizer(prefix):
    model_dir = os.path.join(MODEL_DIR, f"bert_{prefix}", "final")
...
def get_eval_args(prefix, batch_size):
    ...
    return TrainingArguments(
        output_dir=os.path.join(OUTPUT_DIR, f'bert_{prefix}_eval_tmp'),
        per_device_eval_batch_size=batch_size,
        fp16=use_cuda,
        logging_dir=os.path.join(LOGGING_DIR, f"bert_{prefix}_eval"),
    )
...
def save_metrics(metrics, prefix, split_name):
    bert_eval_dir = os.path.join(EVAL_DIR, "bert")
    os.makedirs(bert_eval_dir, exist_ok=True)

    out_path = os.path.join(bert_eval_dir, f"bert_{prefix}_{split_name}_metrics.txt")
...
def save_detailed_evaluation(predictions, labels, prefix):
    ...
    report_path = os.path.join(bert_eval_dir, f"bert_{prefix}_detailed_report.txt")
    ...
    plt.title(f"Confusion Matrix - BERT ({prefix.upper()})")
    plt.xlabel("Predicted label")
    plt.ylabel("True label")
    plt.xticks(rotation=45)
    plt.yticks(rotation=0)
    
    cm_path = os.path.join(bert_eval_dir, f"bert_{prefix}_confusion_matrix.png")
...
def main():
    ...
    for prefix, batch_size in configs:
        print(f"\n{'='*50}")
        print(f"EVALUATING BERT ON: {prefix.upper()}")
        print(f"{'='*50}")
